...
$.mainContent.location.history = {
	_history: [],
	add: function(url){
		this._history.push(url);
		if(this._history.length > 10){
			this._history = this._history.slice(this._history.length - 10);
		}
	},
	back: function(step){
		if(!step){
		...
		if(step>=0 || step < 1-this._history.length){
			return ;
		}
		var url = this._history[this._history.length+step-1];
		this._history = this._history.slice(0, this._history.length+step-2);
		loadPage(url);
	}
};
	...
	$.ajax({
		...
		success : function() {
			...
			$.mainContent.load($.subPagesDirectory + url, null, function (responseText) {
				...
				$.mainContent.animate({ opacity : 1 }, 0).location.history.add(url);
			})//.delay(250).animate({ opacity : 1 }, 0);
		},
		error : function() {
			//window.location.href = $.page404;
			console.log("[404] Page Not Found: \"" + $.subPagesDirectory + url +"\"");
		}
...
$(document).ready(function($){
	$(document).on('click', 'a[href!="#"].load-page', function(e) {
	...
	$(document).on('click', '.go-back', function(e){
		$.mainContent.location.history.back(-1);
	});
});
